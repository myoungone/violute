...
flags.DEFINE_multi_string(
    "input_path", None, help="Path to a directory containing audio files", required=True
)
flags.DEFINE_string(
    "output_path", None, help="Output directory for the dataset", required=True
)
flags.DEFINE_integer(
    "num_signal", 131072, help="Number of audio samples to use during training"
)
flags.DEFINE_integer("channels", 1, help="Number of audio channels")
flags.DEFINE_integer(
    "sampling_rate", 44100, help="Sampling rate to use during training"
)
flags.DEFINE_integer("max_db_size", 100, help="Maximum size (in GB) of the dataset")
flags.DEFINE_multi_string(
    "ext",
    default=["aif", "aiff", "wav", "opus", "mp3", "aac", "flac", "ogg"],
    help="Extension to search for in the input directory",
)
flags.DEFINE_bool("lazy", default=False, help="Decode and resample audio samples.")
...
def load_audio_chunk(
    path: str, n_signal: int, sr: int, channels: int = 1
) -> Iterable[np.ndarray]:
    _, input_channels = get_audio_channels(path)
    channel_map = range(channels)
    if input_channels < channels:
        channel_map = (
            math.ceil(channels / input_channels) * list(range(input_channels))
        )[:channels]
...
def get_audio_length(path: str) -> float:
    process = subprocess.Popen(
        ["ffprobe", "-i", path, "-v", "error", "-show_entries", "format=duration"],
        stdout=subprocess.PIPE,
...
def get_audio_channels(path: str) -> int:
    process = subprocess.Popen(
        ["ffprobe", "-i", path, "-v", "error", "-show_entries", "stream=channels"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    )
    stdout, _ = process.communicate()
...
def process_audio_array(
    audio: Tuple[int, bytes], env: lmdb.Environment, channels: int = 1
) -> int:
...
def process_audio_file(
    audio: Tuple[int, Tuple[str, float]], env: lmdb.Environment
) -> int:
    audio_id, (path, length, channels) = audio
    ae = AudioExample(
        metadata={"path": path, "length": str(length), "channels": str(channels)}
    )
    key = f"{audio_id:08d}"
    with env.begin(write=True) as txn:
        txn.put(
...
def flatmap(
    pool: multiprocessing.Pool, func: Callable, iterable: Iterable, chunksize=None
):
...
def main(argv):
    ...
    if not FLAGS.lazy:
        ...
        processed_samples = map(
            partial(process_audio_array, env=env, channels=FLAGS.channels), chunks
        )
        ...
        processed_samples = map(partial(process_audio_file, env=env), audio_lengths)
        pbar = tqdm(processed_samples)
        n_seconds = 0
        for length in pbar:
            n_seconds += length
