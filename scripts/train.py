...
import rave.core
import rave.dataset
from rave.transforms import add_augmentation, get_augmentations
...
flags.DEFINE_string("result_path", default="models", help="Best model folder")
...
def main(argv):
    ...
    if os.name == "nt" or sys.platform == "darwin":
        num_workers = 0
    train = DataLoader(
        train, FLAGS.batch, True, drop_last=True, num_workers=num_workers
    )
    val = DataLoader(val, FLAGS.batch, False, num_workers=num_workers)

    # CHECKPOINT CALLBACKS
    # validation_checkpoint = pl.callbacks.ModelCheckpoint(
    #    monitor="validation", filename="best-{epoch:04d}"
    # )
    ...
    best_checkpoint = pl.callbacks.ModelCheckpoint(
        ...
        mode="min",
        save_top_k=1,
        dirpath=os.path.join(FLAGS.result_path, FLAGS.name),
        filename="best_model",
    )
    ...
    os.makedirs(os.path.join(FLAGS.result_path, FLAGS.name), exist_ok=True)
    with open(
        os.path.join(FLAGS.result_path, FLAGS.name, "config.gin"), "w"
    ) as config_out:
        config_out.write(gin.operative_config_str())
    ...
    logging.info(f"Run info: {trainer.log_dir}")
    logging.info(
        f"Checkpoint of the best model: {os.path.join(FLAGS.result_path, FLAGS.name, 'best_model.ckpt')}"
    )
