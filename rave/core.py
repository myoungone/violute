...
def setup_gpu():
    return gpu.getAvailable(maxMemory=0.05)
...
def mean_difference(
    target: torch.Tensor, value: torch.Tensor, norm: str = "L1", relative: bool = False
):
    diff = target - value
    if norm == "L1":
        diff = diff.abs().mean()
...
class AudioDistanceV1(nn.Module):
    def __init__(
        self, multiscale_stft: Callable[[], nn.Module], log_epsilon: float
    ) -> None:
        super().__init__()
...
class WeightedInstantaneousSpectralDistance(nn.Module):
    def __init__(
        self, multiscale_stft: Callable[[], MultiScaleSTFT], weighted: bool = False
    ) -> None:
        super().__init__()
        self.multiscale_stft = multiscale_stft()
        self.weighted = weighted

    def phase_to_instantaneous_frequency(self, x: torch.Tensor) -> torch.Tensor:
        x = self.unwrap(x)
        x = self.derivative(x)
        return x
    ...
    def forward(self, target: torch.Tensor, pred: torch.Tensor):
        ...
        for x, y in zip(stfts_x, stfts_y):
            ...
            phase_distance = phase_distance + mean_difference(x_if, y_if, norm="L2")
...
class EncodecAudioDistance(nn.Module):
    def __init__(
        self, scales: int, spectral_distance: Callable[[int], nn.Module]
    ) -> None:
        super().__init__()
        ...
        if self.step_period:
            if self.__counter % self.step_period == 0:
                filename = os.path.join(
                    self.dirpath, f"epoch_{self.__counter}{self.FILE_EXTENSION}"
                )
