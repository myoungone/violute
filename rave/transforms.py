...
class Resample(Transform):
    ...
    def __call__(self, x: np.ndarray):
        return torchaudio.functional.resample(
            torch.from_numpy(x).float(), self.orig_sr, self.target_sr
        ).numpy()
...
class RandomPitch(Transform):
    def __init__(
        self, n_signal, pitch_range=[0.7, 1.3], max_factor: int = 20, prob: float = 0.5
    ):
...
class Compress(Transform):
    ...
    def __call__(self, x: torch.Tensor):
        x = torchaudio.sox_effects.apply_effects_tensor(
            torch.from_numpy(x).float(), self.sr, [self.sox_args]
        )[0].numpy()
        return x
...
class RandomCompress(Transform):
    def __init__(
        ...
        assert prob >= 0.0 and prob <= 1.0, "prob must be between 0. and 1."
    ...
    def __call__(self, x: torch.Tensor):
        ...
        if perform:
            amp_factor = (
                torch.rand((1,)) * (self.amp_range[1] - self.amp_range[0])
                + self.amp_range[0]
            )
            x_aug = torchaudio.sox_effects.apply_effects_tensor(
                torch.from_numpy(x).float(),
                self.sr,
                [
                    [
                        "compand",
                        f"{self.attack},{self.release}",
                        f"6:-80,{self.threshold},{float(amp_factor)}",
                    ]
                ],
            )[0].numpy()
            return x_aug
...
class RandomGain(Transform):
    def __init__(
        self, gain_range: Tuple[int, int] = [-6, 3], prob: float = 0.5, limit=True
    ):
        assert prob >= 0.0 and prob <= 1.0, "prob must be between 0. and 1."
